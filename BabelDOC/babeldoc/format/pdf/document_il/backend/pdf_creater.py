...
class PDFCreater:
    ...
    def subset_fonts_in_subprocess(
        ...
        process = subprocess.Popen(
            [sys.executable, "-c", f"from babeldoc.format.pdf.document_il.backend.pdf_creater import _subset_fonts_process; _subset_fonts_process('{temp_input}', '{temp_output}')"],
        ...
        while process.poll() is None:
            if time.time() - start_time > timeout:
                ...
                process.terminate()
                try:
                    process.wait(5)  # Give it 5 seconds to clean up
                    if process.is_alive():
    ...
    def save_pdf_with_timeout(
        ...
        if sys.platform == "win32":
            creationflags = subprocess.CREATE_NO_WINDOW
        process = subprocess.Popen(
            [sys.executable, "-c", f"from babeldoc.format.pdf.document_il.backend.pdf_creater import _save_pdf_clean_process; _save_pdf_clean_process('{temp_input}', '{temp_output}', {garbage}, {deflate}, {clean}, {deflate_fonts}, {linear})"],
            creationflags=creationflags,
        )
    ...
    def write(
        ...
        try:
            ...
            mono_out_path = translation_config.get_output_file_path(
                f"{basename}_NKH Trans_MONO.pdf",
            )
            ...
            with self.translation_config.progress_monitor.stage_start(
                ...
                if not translation_config.no_dual:
                    dual_out_path = translation_config.get_output_file_path(
                        f"{basename}_NKH Trans_DUAL.pdf",
                    )
                    translation_config.raise_if_cancelled()
