...
def merge_watermark_doc(
    ...
    original_name = no_watermark_pdf_path.stem.replace(".no_watermark", "")
    if "mono" in no_watermark_pdf_path.name:
        new_file_name = f"{original_name}.NKH Trans_Mono.pdf"
    else:
        new_file_name = f"{original_name}.NKH Trans_Dual.pdf"
    new_save_path = no_watermark_pdf_path.with_name(new_file_name)
